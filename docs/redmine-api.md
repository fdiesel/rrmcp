...
| Journals            | Alpha     | 5.0   | Fetched via issue include; notes updatable via PUT         |
...
- Issues: `journals,relations,attachments,changesets,watchers,children,allowed_statuses`
...
DELETE /memberships/{id}.json          Remove membership
```

## Issue Categories API

```
GET    /projects/{id}/issue_categories.json  List categories
POST   /projects/{id}/issue_categories.json  Create category
GET    /issue_categories/{id}.json           Get category
PUT    /issue_categories/{id}.json           Update category
DELETE /issue_categories/{id}.json           Delete category
```
...
```
GET    /issues/{id}/relations.json     List relations for an issue
...
Attach to an issue by including the upload token in the `uploads[]` array on a PUT/POST to the issue.
...
Journals are fetched via `?include=journals` on issue requests. Journal notes can be updated:

```
PUT    /journals/{id}.json             Update journal (notes field only)
```

Journals are auto-generated by Redmine on issue changes and cannot be created or deleted via API.

## Groups API

```
GET    /groups.json                    List groups (admin only)
POST   /groups.json                    Create group (admin only)
GET    /groups/{id}.json               Get group
PUT    /groups/{id}.json               Update group (admin only)
DELETE /groups/{id}.json               Delete group (admin only)
...
```
GET    /issue_statuses.json            List all issue statuses
```

## Trackers API

```
GET    /trackers.json                  List all trackers
```
...
```
GET    /news.json                      List global news
GET    /projects/{id}/news.json        List project news
```

Note: News supports listing only — no create/update/delete via API.

## Files API

```
GET    /projects/{id}/files.json       List files in a project
POST   /projects/{id}/files.json       Add a file to a project (uses upload token)
```
...
- Journals are auto-generated on issue changes; only their `notes` field can be updated via `PUT /journals/{id}.json`
- File upload (attachments) is a two-step process: upload first (`POST /uploads.json`), then attach to an issue
- `X-Redmine-Switch-User` requires admin privileges; the impersonated user must exist and be active
